...
def blind_based(url):
        ...
        while True:
            ...
            if choice is 1:
                    table_name(req,db_name)
            else:
                    print("Returning")
        
def perform(url):
        fullbody = fetch(url,"'")
        flag=error(url,fullbody)
        print("*"*80)
...
def sql():
	print("*"*80)
	print("Welcome to the Automated SQL Injection Module")
	req  = input("Enter the URL:\n")
	if req == '\0':
		print("Please Specify the proper URL")
	try:
